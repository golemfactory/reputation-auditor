...
def process_disk_benchmark(data_list):
    ...
    existing_providers = {provider.node_id: provider for provider in Provider.objects.filter(
        node_id__in=provider_node_ids)}

    # Handling providers not already in the database
    new_providers = [Provider(node_id=node_id)
                     for node_id in provider_node_ids if node_id not in existing_providers]
    if new_providers:
        Provider.objects.bulk_create(new_providers)
        # Update cache of existing providers
        existing_providers.update(
            {provider.node_id: provider for provider in new_providers})
    ...
    for data in data_list:
        ...
        disk_benchmark_objects.append(DiskBenchmark(
            ...
            latency_95th_percentile_ms=float(
                data['latency_95th_percentile_ms']),
...
def process_cpu_benchmark(data_list):
    ...
    for data in data_list:
        ...
        cpu_benchmark_objects.append(CpuBenchmark(
            ...
            total_time_sec=data['total_time_sec'].replace(
                "s", ""),  # Remove 's' if present
...
def process_memory_benchmark(data_list):
    ...
    for data in data_list:
        ...
        memory_benchmark_objects.append(MemoryBenchmark(
            ...
            total_data_transferred_mi_b=float(
                data['total_data_transferred_mi_b']),
...
def process_network_benchmark(network_data):
    ...
    providers = {provider.node_id: provider for provider in Provider.objects.filter(
        node_id__in=provider_ids)}
